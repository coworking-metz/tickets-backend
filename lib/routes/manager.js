...
export function managerRouter() {
  ...
  router.get('/members/:id/presences', w(async (req, res) => {
    const user = await mongo.db.collection('users').findOne({_id: req.params.id})
    if (!user) {
      throw createError(404)
    }

    const presences = chain(user.profile.presences)
      .map(presence => ({
        date: presence.date,
        amount: presence.amount,
        type: isPresenceDuringAbo(presence.date, user.profile.abos) ? 'SUBSCRIPTION' : 'TICKET'
      }))
      .orderBy(['date'], ['desc'])
      .value()
  ...
  router.get('/members/:id/subscriptions', w(async (req, res) => {
    const user = await mongo.db.collection('users').findOne({_id: req.params.id})
    if (!user) {
      throw createError(404)
    }

    const subscriptions = chain(user.profile.abos)
      .map(subscription => ({
  ...
  router.get('/members/:id/tickets', w(async (req, res) => {
    const user = await mongo.db.collection('users').findOne({_id: req.params.id})
    if (!user) {
      throw createError(404)
    }

    const tickets = chain(user.profile.tickets)
      .map(ticket => ({
