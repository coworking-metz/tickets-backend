...
async function createRoutes() {
  ...
  router.get('/key-box/code/:ent?', w(async (req, res) => {
    const ent = (req.params.ent || 'MAIN').toUpperCase()
    const envKey = `KEY_BOX_CODE_${ent}`
    ...
    if (!code) {
      throw createHttpError(501, `Le code de la boîte à clé (${ent}) n'est pas configuré`)
    }
