...
async function createRoutes() {
  ...
  router.get('/unlock-gate/options', w(ensureAccess), w(async (_req, res) => {
    const identifier = process.env.UNLOCK_GATE_BEACON_IDENTIFIER
    const uuid = process.env.UNLOCK_GATE_BEACON_UUID
    const major = process.env.UNLOCK_GATE_BEACON_MAJOR
    const minor = process.env.UNLOCK_GATE_BEACON_MINOR

    if (!identifier || !uuid || !major || !minor) {
      throw createHttpError(501, 'Missing beacon config for unlocking the gate')
    }
