import {Router} from 'express'
import Papa from 'papaparse'

import w from '../util/w.js'

import {parseFromTo} from '../dates.js'

import {
  computeStats,
  computePeriodsStats,
  computePeriodIncomes,
  asCsv,
  PERIODS_TYPES,
  computePeriodAttendance
} from '../stats.js'
import {ensureAdmin, multiAuth} from '../auth.js'

async function createRoutes() {
  const app = new Router()

  app.get('', w(async (req, res) => {
    const stats = await computeStats()
    res.send(stats)
  }))

  app.get('/:periodType', w(async (req, res) => {
    const {periodType} = req.params

    if (!PERIODS_TYPES.has(periodType)) {
      return res.sendStatus(404)
    }

    const {from, to} = parseFromTo(req.query.from, req.query.to)

    const stats = await computePeriodsStats(periodType, {
      includesCurrent: req.query.includesCurrent === '1',
      from,
      to
    })

    if (req.query.format === 'csv') {
      return res.type('text/csv').send(
        Papa.unparse(stats.map(s => asCsv(s)))
      )
    }

    res.send(stats)
  }))

  app.get('/incomes/:periodType', w(async (req, res) => {
    const {periodType} = req.params

    if (!PERIODS_TYPES.has(periodType)) {
      return res.sendStatus(404)
    }

    const {from, to} = parseFromTo(req.query.from, req.query.to)

    const stats = await computePeriodIncomes(periodType, from, to)

    if (req.query.format === 'csv') {
      return res.type('text/csv').send(
        Papa.unparse(stats.map(s => ({
          date: s.date,
          type: s.type,
          used_tickets: s.data.tickets.count,
          days_abos: s.data.subscriptions.count,
          incomes: s.data.tickets.amount + s.data.subscriptions.amount
        })))
      )
    }

    res.send(stats)
  }))

  app.get('/attendance/:periodType', w(multiAuth), w(ensureAdmin), w(async (req, res) => {
    const {periodType} = req.params

    if (!PERIODS_TYPES.has(periodType)) {
      return res.sendStatus(404)
    }

    const {from, to} = parseFromTo(req.query.from, req.query.to)

    const stats = await computePeriodAttendance(periodType, from, to)

    res.send(stats)
  }))

  return app
}

const routes = await createRoutes()
export default routes
