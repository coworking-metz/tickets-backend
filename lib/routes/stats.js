...
import {ensureAdmin, multiAuth} from '../auth.js'
...
async function createRoutes() {
  ...
  app.get('/:periodType', w(multiAuth), w(ensureAdmin), w(async (req, res) => {
  ...
  app.get('/activity/:periodType', w(multiAuth), w(ensureAdmin), w(async (req, res) => {
    const {periodType} = req.params
  ...
  app.get('/incomes/:periodType', w(multiAuth), w(ensureAdmin), w(async (req, res) => {
    const {periodType} = req.params
  ...
  app.get('/income/:periodType', w(multiAuth), w(ensureAdmin), w(async (req, res) => {
    const {periodType} = req.params
    ...
    const stats = await computePeriodIncome(periodType, from, to)

    res.send(stats)
  }))

  app.get('/usage/:periodType', w(multiAuth), w(ensureAdmin), w(async (req, res) => {
    const {periodType} = req.params
    ...
    const stats = await computePeriodUsage(periodType, from, to)

    res.send(stats)
  }))

  app.get('/presences/month/:year/:month', w(multiAuth), w(ensureAdmin), w(async (req, res) => {
    ...
    await pMap(users, async user => {
      const userStats = {user: user.email, ticket: 0, subscription: 0, presences: 0, dates: {}}
      const memberActivity = await Activity.getMemberActivity(user._id)

      for (const activityEntry of memberActivity) {
        const {date, value, type} = activityEntry
  ...
  app.get('/attendance/:periodType', w(multiAuth), w(ensureAdmin), w(async (req, res) => {
    const {periodType} = req.params
    ...
    const stats = await computePeriodAttendance(
      periodType,
