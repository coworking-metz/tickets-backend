...
function computeAboEnd(aboStart) {
  // On ajoute 6 heures avant d'être sûr de ne pas être impacté par les changements d'heures
  return formatDate(sub(add(new Date(aboStart), {months: 1, hours: 6}), {days: 1}))
}
      ...
      $push: {
        'profile.abos': {
          $each: range(quantity).map(i => {
            const aboStart = formatDate(add(new Date(startDate), {months: i}))
            const aboEnd = computeAboEnd(aboStart)
            return {purchaseDate, aboStart, aboEnd}
          })
        }
      }
    }
...
async function addMembership(userId, purchaseDate, membershipYear) {
  await mongo.db.collection('users').updateOne(
    {_id: userId},
    {
      $push: {
...
function isPresenceDuringAbo(presenceDate, abos) {
  return abos.some(
    abo => abo.aboStart <= presenceDate && presenceDate <= abo.aboEnd
  )
}
