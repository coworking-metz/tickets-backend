...
function computeUserStats(user) {
  ...
  if (lastMembership) {
    userStats.lastMembership = lastMembership.membershipStart.slice(0, 4)
  }
...
async function computeIncomes(periodType, from, to) {
  ...
  users.forEach(user => {
    user.profile.presences.forEach(p => {
      const {date, amount} = p
  ...
  return periods.map(period => {
    ...
    const incomes = days.reduce((sum, dayPeriod) => {
      const day = dayPeriod[0]
      const dateEntries = datesIndex[day] || []
      const tickets = dateEntries.filter(e => !e.abo).reduce((ticketsSum, e) => {
        return ticketsSum + e.amount
      }, 0)
      const activeAbo = users.filter(u => isPresenceDuringAbo(day, u.profile.abos)).length
      return sum + tickets * 6 + activeAbo * 2
    }, 0)

    return {from: period[0], to: period[1], incomes}
  })
}
