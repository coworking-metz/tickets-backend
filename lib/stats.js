...
const SINGLE_TICKET_PRICE_IN_EUR = 8
 ...
 *
 * SUBSCRIPTIONS
 * For each day, get all subscriptions covering this day (stored them for later)
 * then get the daily price of each subscription, divide their price by their period (price / (ended - started))
 * then sum all the daily prices to get the total subscription income for this day
 *
 * TICKETS
 * For each day, get all members presents (from their activity)
 * then remove the ones who have a matching active subscription (in the subscriptions previously fetched)
 *
 * For each member remaining, we need to find what price their ticket was (if they had sufficient balance)
...
export async function computePeriodIncomes(periodType, from, to) {
  const periods = getPeriods(periodType, from, to)
  const membersActivityCache = new Map()
  const membersTicketsOrdersCache = new Map()
  ...
  const incomeByPeriod = await Promise.all(periods.map(async ([started, ended]) => {
    ...
    const incomeByDay = await Promise.all(days.map(async day => {
      ...
      const pricedTickets = await Promise.all(ticketsActivity.map(async ticketActivity => {
        let memberTicketsOrders = membersTicketsOrdersCache.get(ticketActivity.member)
        if (!memberTicketsOrders) {
          memberTicketsOrders = await Ticket.getMemberTicketsOrders(ticketActivity.member)
          membersTicketsOrdersCache.set(ticketActivity.member, memberTicketsOrders)
        }
        ...
        let memberTicketActivity = membersActivityCache.get(ticketActivity.member)
        if (!memberTicketActivity) {
          const memberActivity = await Activity.getMemberActivity(ticketActivity.member)
          memberTicketActivity = memberActivity.filter(activity => activity.type === 'ticket')
          membersActivityCache.set(ticketActivity.member, memberTicketActivity)
        }

        const pastTicketActivity = memberTicketActivity.filter(activity => activity.date < day)
        ...
        if (totalQuantity <= pastTotalUsed) {
          ...
          return {
            ...
            amount: 0,
            debt: {
              value: ticketActivity.value,
              amount: ticketActivity.value * SINGLE_TICKET_PRICE_IN_EUR
            }
          }
        }

        if (totalQuantity <= pastTotalUsed + ticketActivity.value) {
          // Ticket has been partly consumed but couldn't be fully paid. It should be marked as debt
          // but also we should let the system compute the remaining amount
          const debtValue = Math.min(1, Math.abs(pastTotalUsed + ticketActivity.value - totalQuantity))

          ticketActivity.value -= debtValue
          ticketActivity.debt = {
            value: debtValue,
            amount: debtValue * SINGLE_TICKET_PRICE_IN_EUR
          }
        }
      ...
      const ticketsSummary = tickets.reduce((acc, ticket) => {
        if (ticket.debt) {
          return {
            ...acc,
            debt: {
              count: acc.debt.count + ticket.debt.value,
              amount: acc.debt.amount + ticket.debt.amount
            }
          }
        }
...
export async function computePeriodAttendance(periodType, from, to) {
  ...
  const attendanceByPeriod = await Promise.all(periods.map(async ([started, ended]) => {
    ...
    const attendanceByDay = await Promise.all(days.map(async day => {
      ...
      const activeSubscriptions = await Subscription.findActiveSubscriptionsByDate(day)
      const subscriptionActivity = membersActivity
        .map(activity => {
          const activeSubscription = activeSubscriptions.find(subscription => subscription.memberId === activity.member)
      ...
      const pricedTickets = await Promise.all(ticketsActivity.map(async ticketActivity => {
        ...
        if (totalQuantity <= pastTotalUsed + ticketActivity.value) {
          // Ticket has been partly consumed but couldn't be fully paid. It should be marked as debt
          // but also we shoult let the system compute the remaining amount
          const debtValue = Math.min(1, Math.abs(pastTotalUsed + ticketActivity.value - totalQuantity))
    ...
    // eslint-disable-next-line unicorn/no-array-reduce
    const attendingMembersSet = Object.entries(attendanceByDay.reduce((acc, day) => {
      const {subscriptions, tickets} = day

      for (const subscription of subscriptions) {
        const member = acc[subscription.member]

        if (member) {
          member.subscriptions.push(subscription)
        } else {
          acc[subscription.member] = {
            tickets: [],
      ...
      for (const ticket of tickets) {
        const member = acc[ticket.member]
    ...
    const attendingMembers = await Promise.all([...attendingMembersSet].map(async ([userId, {tickets, subscriptions}]) => {
      ...
      // eslint-disable-next-line unicorn/no-array-reduce
      const subscriptionActivity = subscriptions.reduce((acc, {value, subscription}) => {
        const endDate = computeSubcriptionEndDate(subscription.startDate)
        const periodInDays = differenceInDays(
          new Date(endDate),
          new Date(subscription.startDate)
        )
        const amount = subscription.price / periodInDays * value
