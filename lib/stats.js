...
export async function computePeriodUsage(periodType, from, to) {
  ...
  const usageByPeriod = await Promise.all(periods.map(async ([started, ended]) => {
    ...
    const usageByDay = await Promise.all(days.map(async day => {
      ...
      const pricedTickets = await pMap(ticketsActivity, getTicketActivityAmount, {concurrency: 10})
    ...
    const periodMembersSet = new Set()

    for (const day of usageByDay) {
      periodCharges += day.charges

      for (const ticket of day.tickets) {
        addSummaryTicket(periodTickets, ticket)

        if (!periodMembersSet[ticket.memberId]) {
          periodMembersSet[ticket.memberId] = {
            tickets: initializeTicketsSummary(),
            subscriptions: initializeSubscriptionsSummary()
          }
        }

        addSummaryTicket(periodMembersSet[ticket.memberId].tickets, ticket)
      }
...
const getTicketActivityAmount = async ticketActivity => {
...
const addSummaryTicket = (summary, ticket) => {
  summary.count += ticket.value
  summary.amount += ticket.amount
  summary.debt.count += ticket.debt ? ticket.debt.value : 0
  summary.debt.amount += ticket.debt ? ticket.debt.amount : 0
}
...
export async function computePeriodAttendance(periodType, from, to) {
  ...
  const attendanceByPeriod = await Promise.all(periods.map(async ([started, ended]) => {
    ...
    const attendanceByDay = await Promise.all(days.map(async day => {
      const membersActivity = await Activity.getActivityByDate(day)
      const pricedActivity = await pMap(membersActivity, async activity => ({
        ...(await Member.getComputedMemberActivityAt(activity.member, day)),
        memberId: activity.member
      }), {concurrency: 10})
