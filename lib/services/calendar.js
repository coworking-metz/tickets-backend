...
import {getNumberFromSeed, shuffleWithSeed} from '../util/random.js'
...
export const getAllEvents = async (req, res) => {
  ...
  res.send(allEvents.flat()
    ...
    .map(event => {
      const {shufflePicturesOrder, ...rest} = event
      ...
      return {
        ...rest,
        id,
        pictures: shufflePicturesOrder ? shuffleWithSeed(event.pictures, id) : event.pictures
      }
    })
...
const formatICALEvent = event => {
  ...
  const {config, text: descriptionWithoutConfig} = extractEventConfig(description)

  console.log({config})

  const strippedDescription = descriptionWithoutConfig
  ...
  return {
    ...
    shufflePicturesOrder: Boolean(config.shufflePicturesOrder)
  }
}

/**
 * @typedef {object} EventConfig
 * @property {boolean} [shufflePicturesOrder] - Whether the event's pictures should be shown in a shuffled order
 */

/**
 * Extract a JSON config from HTML comments in a text, and remove those comments from it.
 * Lets an event writer configure how their event is rendered without exposing raw markup to the end user,
 * e.g. `<!-- { "shufflePicturesOrder": true } -->`.
 *
 * @param {string} text - The text to extract the config from
 * @returns {{config: EventConfig, text: string}} The merged config and the text without the config comments
 */
const extractEventConfig = text => {
  const regexp = /<!--\s*({[\s\S]*?})\s*-->/g
  let config = {}

  const cleanedText = text.replaceAll(regexp, (_, rawConfig) => {
    try {
      const normalizedConfig = rawConfig.replaceAll(/[“”]/g, '"').replaceAll(/[‘’]/g, '\'')
      config = {...config, ...JSON.parse(normalizedConfig)}
    } catch {
      // Malformed config: ignore it, but still strip the comment from the description below
    }

    return ''
  }).replaceAll(/\n{3,}/g, '\n\n')

  return {config, text: cleanedText}
}
