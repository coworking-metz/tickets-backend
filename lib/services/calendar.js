...
const fetchCoworkingEvents = async () => {
  ...
  const now = new Date()
  const icsEvents = icalExpander.between(sub(now, {months: 1}), add(now, {months: 1}))
...
const formatICALEvent = event => {
  const urls = uniq(extractUrls(event.description))
  let strippedDescription = (event.description || '')
