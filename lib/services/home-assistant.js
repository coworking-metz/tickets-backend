...
const INTERCOM_UNLOCK_BUTTON_ID = 'button.open_space_esp32_poe_iso_unlock_interphone_door'
...
const AIR_CONDITIONER_NORTH_ID = 'climate.open_space_airton_air_conditioner_north'
const AIR_CONDITIONER_SOUTH_ID = 'climate.open_space_airton_air_conditioner_south'

export const AIR_CONDITIONER_TARGET_TEMPERATURE_RANGE = {min: 18, max: 30}

export const AIR_CONDITIONER_ENTITY_IDS = {
  north: AIR_CONDITIONER_NORTH_ID,
  south: AIR_CONDITIONER_SOUTH_ID
}

const AIR_CONDITIONER_ACTIVE_STATE_POLLING_TIMEOUT = 10_000 // 10 seconds
const AIR_CONDITIONER_ACTIVE_STATE_POLLING_INTERVAL = 500 // 0.5 second
...
export const getCurrentState = async () => {
  const entities = await getEntitiesState(new Set([
    ...
    AIR_CONDITIONER_NORTH_ID,
    AIR_CONDITIONER_SOUTH_ID,
  ...
  const airConditionerNorth = entities.find(({entity_id}) => entity_id === AIR_CONDITIONER_NORTH_ID)
  const airConditionerSouth = entities.find(({entity_id}) => entity_id === AIR_CONDITIONER_SOUTH_ID)
  ...
  return {
    ...
    airConditioners: {
      north: formatAirConditioner(airConditionerNorth),
      south: formatAirConditioner(airConditionerSouth)
    },
    flexDesks: {
      a: {
...
export const pressIntercomButton = async () => {
  ...
  const [changedState] = await got.post('api/services/button/press', {
    ...
    json: {
      entity_id: INTERCOM_UNLOCK_BUTTON_ID
    }
  }).json()
...
export const setAirConditioningActive = async (id, active) => {
  if (!HOME_ASSISTANT_BASE_URL || !HOME_ASSISTANT_LONG_LIVED_TOKEN) {
    throw createError(501, 'Home Assistant service not configured')
  }

  const entityId = AIR_CONDITIONER_ENTITY_IDS[id]
  if (!entityId) {
    throw createError(400, `Le climatiseur "${id}" n'existe pas`)
  }

  await got.post(`api/services/climate/turn_${active ? 'on' : 'off'}`, {
    prefixUrl: HOME_ASSISTANT_BASE_URL,
    timeout: {
  ...
  const changedState = await waitForAirConditioningActiveState(entityId, active, Date.now() + AIR_CONDITIONER_ACTIVE_STATE_POLLING_TIMEOUT)

  return formatAirConditioner(changedState)
}

export const setAirConditioningTargetTemperature = async (id, temperature) => {
  if (!HOME_ASSISTANT_BASE_URL || !HOME_ASSISTANT_LONG_LIVED_TOKEN) {
    throw createError(501, 'Home Assistant service not configured')
  }
  ...
  const [changedState] = await got.post('api/services/climate/set_temperature', {
    prefixUrl: HOME_ASSISTANT_BASE_URL,
    timeout: {
      request: 5000 // 5 seconds
    },
  ...
  if (!changedState) {
    throw createError(503, 'Le climatiseur ne répond pas. Vous pouvez réessayer dans quelques instants.')
  }
...
const isAirConditioningActive = state => includes(['auto', 'cool', 'dry', 'fan_only', 'heat'], state)

/**
 * Poll the entity state until it reflects the expected active state,
 * giving up once the deadline has passed
 */
const waitForAirConditioningActiveState = async (entityId, active, deadline) => {
  const [entity] = await getEntitiesState(new Set([entityId]))

  if (isAirConditioningActive(entity?.state) === active) {
    return entity
  }
  ...
  await delay(AIR_CONDITIONER_ACTIVE_STATE_POLLING_INTERVAL)
  return waitForAirConditioningActiveState(entityId, active, deadline)
}

const formatAirConditioner = entity => ({
  id: entity?.entity_id === AIR_CONDITIONER_NORTH_ID ? 'north' : (entity?.entity_id === AIR_CONDITIONER_SOUTH_ID ? 'south' : null),
  active: isAirConditioningActive(entity?.state),
  targetTemperature: isNil(entity?.attributes?.temperature) ? null : Number(entity?.attributes?.temperature),
  currentTemperature: isNil(entity?.attributes?.current_temperature) ? null : Number(entity?.attributes?.current_temperature),
  minTemperature: isNil(entity?.attributes?.min_temp) ? null : Math.max(entity?.attributes?.min_temp, AIR_CONDITIONER_TARGET_TEMPERATURE_RANGE.min),
  maxTemperature: isNil(entity?.attributes?.max_temp) ? null : Math.min(entity?.attributes?.max_temp, AIR_CONDITIONER_TARGET_TEMPERATURE_RANGE.max),
  fanSpeed: isNil(entity?.attributes?.fan_mode) ? null : entity?.attributes?.fan_mode
})
