...
export const getCurrentState = async () => {
  const carbonDioxideSensor = `${BIG_NETATMO_STATION_ID}_co2`
  ...
  const deckDoor = entities.find(entity => entity.entity_id === DECK_DOOR_ID)
  const co2Sensor = entities.find(entity => entity.entity_id === carbonDioxideSensor)
...
export const pressIntercomButton = async () => {
  ...
  if (!changedState) {
    throw createError(503, 'Le déverrouillage de la porte a échoué.')
  }
