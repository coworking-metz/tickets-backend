...
export async function openParkingBarrier() {
  ...
  if (Number.isNaN(Number(PORTAPHONE_RELAY_DOOR_INDEX))) {
    throw createError(501, 'Portaphone door index not properly configured: require a number')
  }
  ...
  try {
    ...
    const payload = JSON.stringify({
      ...
      body: {
        token,
        door: Number(PORTAPHONE_RELAY_DOOR_INDEX)
      }
    })
