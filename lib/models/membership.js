import mongo from '../util/mongo.js'
import {customAlphabet} from 'nanoid'
import {endOfYear, isAfter} from 'date-fns'
import {chain} from 'lodash-es'

const nanoid = customAlphabet('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz')

export function formatMembership(membershipFromDatabase) {
  return {
    _id: membershipFromDatabase._id,
    orderReference: membershipFromDatabase.orderReference,
    membershipStart: membershipFromDatabase.membershipStart,
    membershipEnd: computeMembershipEndDate(membershipFromDatabase.membershipStart),
    purchased: membershipFromDatabase.purchaseDate,
    amount: membershipFromDatabase.price
  }
}

export async function getMemberMemberships(memberId) {
  const memberships = await mongo.db.collection('memberships').find({memberId}).toArray()
  return chain(memberships)
    .map(m => formatMembership(m))
    .orderBy(['membershipStart'], ['desc'])
    .value()
}

export async function addMembershipToMember(memberId, membershipStart, purchase, purchaseQuantity = 1) {
  const payload = {
    _id: null,
    memberId,
    membershipStart, ...purchase
  }

  const payloads = []
  for (let i = 0; i < purchaseQuantity; i++) {
    payload._id = nanoid(17)
    payloads.push({...payload})
  }

  if (!purchase.migratedFromUsersCollection) {
    await addMembershipToMemberLegacy(memberId, purchase.purchaseDate, membershipStart) // For data consistency
  }

  return Promise.all(payloads.map(async p => {
    await mongo.db.collection('memberships').insertOne(p)
    return p
  }))
}

export async function addMembershipToMemberLegacy(memberId, purchaseDate, membershipStart) {
  await mongo.db.collection('users').updateOne(
    {_id: memberId},
    {
      $push: {
        'profile.memberships': {
          purchaseDate,
          membershipStart
        }
      }
    }
  )
}

export async function updateMembership(_id, set) {
  const updatedMembership = await mongo.db.collection('memberships').findOneAndUpdate(
    {_id},
    {$set: set},
    {returnDocument: 'after'}
  )

  // DEPRECATED - we should avoid updating the users collection directly
  await mongo.db.collection('users').updateOne(
    {
      _id: updatedMembership.value.memberId,
      'profile.memberships.purchaseDate': updatedMembership.value.purchaseDate
    },
    {
      $set: {
        'profile.memberships.$.membershipStart': set.membershipStart
      }
    }
  ).catch(() => {
    // Ignore errors from legacy update
  })

  return updatedMembership.value
}

export async function removeMembership(_id) {
  const removedMembership = await mongo.db.collection('memberships').findOneAndDelete(
    {_id},
    {returnDocument: 'before'}
  )

  // DEPRECATED - we should avoid updating the users collection directly
  await mongo.db.collection('users').updateOne(
    {
      _id: removedMembership.value.memberId,
      'profile.memberships.purchaseDate': removedMembership.value.purchaseDate
    },
    {
      $pull: {
        'profile.memberships': {
          purchaseDate: removedMembership.value.purchaseDate
        }
      }
    }
  )

  return removedMembership.value
}

/**
 * Calculate the unit price of a membership based on its date.
 *
 * @param {Date} date - The date the membership was purchased.
 * @returns {number} The unit price of the membership. If the date is after January 1, 2014, the price is 10; otherwise it's 0.
 */
export function membershipUnitPriceByDate(date) {
  if (isAfter(new Date(date), new Date('2014-01-01'))) {
    return 10
  }

  return 0
}

export function computeMembershipEndDate(membershipStartDate) {
  const startDate = new Date(membershipStartDate)

  // Starting November 1st, membership also covers the following year
  if (startDate.getMonth() >= 10) {
    startDate.setFullYear(startDate.getFullYear() + 1)
  }

  return endOfYear(startDate).toISOString().slice(0, 10)
}

export async function getMembershipsByDate(date) {
  const memberships = await mongo.db.collection('memberships').find({purchaseDate: date}).toArray()
  return chain(memberships)
    .map(m => formatMembership(m))
    .orderBy(['purchased'], ['desc'])
    .value()
}
