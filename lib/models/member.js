  ...
  await mongo.db.collection('users').updateOne(
    {_id: memberId},
    {
      $set: {
        ...
        firstName,
        lastName,
        birthDate: new Date(Date.parse(dateNaissance)).toISOString().slice(0, 10),
        trialDay,
...
export async function computeMemberActivityCoverage(memberId) {
  ...
  // to be in line with FIFO consumption and raw activity order
  const sortedTickets = ticketsOrders.reverse()
  ...
  const activityWithCoverage = rawActivity.map(a => {
    ...
    const ticketActivity = consumeTicketsByActivity(sortedTickets, {
      type: 'ticket',
      date: a.date,
      value: duration,
      coverage: {
