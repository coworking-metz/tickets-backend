...
import {differenceInMinutes, isSameDay, setYear, sub} from 'date-fns'

import mongo from '../util/mongo.js'
import {buildPictureUrl, buildPolaroidUrl, getAnonymousPictureUrl, getUser as getWpUser} from '../util/wordpress.js'

import {computeSubscriptionEndDate, computeBalance, isPresenceDuringAbo} from '../calc.js'
...
async function notifyUserOnArrival(user, newHeartbeat) {
  ...
  if (isArriving) {
    ...
    // Notify if the user is missing a membership
...
export async function setMemberMeta(memberId, key, value) {
  await mongo.db.collection('users').updateOne(
    {_id: memberId},
    {$set: {[`meta.${key}`]: value}}
  )
...
export async function getMemberMeta(memberId, key) {
  const user = await mongo.db.collection('users').findOne(
    {_id: memberId},
    {projection: {[`meta.${key}`]: 1}}
  )
