...
import renderSettleBalanceDebt from '../emails/settle-balance-debt.js'
...
import {computeMembershipEndDate, getMemberMemberships} from './membership.js'
...
export async function isMemberAttending(memberId) {
  const user = await getUserById(memberId)
  return Boolean(user?.profile?.heartbeat) && differenceInMinutes(new Date(), new Date(user.profile.heartbeat)) <= Device.LAST_SEEN_DELAY_IN_MIN
}
...
export async function getVotingMembers(minActivity = 20) {
  ...
  const members = await mongo.db.collection('users').aggregate([
    {
      $lookup: {
    ...
    {
      $addFields: {
        activity: {$sum: '$activities.value'}
      }
    },
    {
      $match: {
        activity: {$gte: minActivity}
      }
    },
    {
      $project: {
...
export async function recomputeBalance(memberId) {
  ...
  const memberActivities = await computeMemberActivityCoverage(memberId)
  const balance = await computeBalance(user, memberActivities)
...
export function computeMembershipOk(lastMembership) {
  ...
  return Boolean(lastMembership && lastMembership >= currentYear)
}
...
export async function computeMemberFromUser(user, options = {}) {
  ...
  const memberships = await getMemberMemberships(user._id)
  const lastMembershipYear = computeLastMembership(memberships)
  member.lastMembership = lastMembershipYear
  const isMembershipOk = computeMembershipOk(lastMembershipYear)
  member.membershipOk = isMembershipOk

  if (withActivity) {
    const memberActivity = await Activity.getMemberRawActivity(user._id)
    const sixMonthsAgo = sub(new Date(), {months: 6}).toISOString().slice(0, 10)

    const sixMonthsActivity = chain(memberActivity)
      .filter(a => a.date >= sixMonthsAgo)
      .sumBy(a => a.overrideValue ?? a.value)
      .value()

    member.activity = sixMonthsActivity
    member.activeUser = sixMonthsActivity >= 20

    const totalActivity = sumBy(memberActivity, a => a.overrideValue ?? a.value)
  ...
  if (withAbos) {
    const subscriptions = await Subscription.getMemberSubscriptions(user._id)
    member.abos = subscriptions.map(s => ({
      purchaseDate: s.purchased,
      aboStart: s.started,
      aboEnd: s.ended,
      current: today >= s.started && today <= s.ended
    }))

    const lastSubscription = maxBy(subscriptions, 'ended')
    if (today <= lastSubscription?.ended) {
      member.lastAboEnd = lastSubscription.ended
    }
  }
  ...
  if (isArriving) {
    ...
    notifyDepletedBalance(user, newHeartbeat).catch(error => {
      notifyOnSignal(`Impossible de notifier ${user.email} pour son solde insuffisant :\n${error.message}`)
        .catch(notifyError => {
          ...
          console.debug('Unable to notify about depleted balance email notification', notifyError)
        })
    })
    ...
    notifyMissingMembership(user).catch(error => {
      notifyOnSignal(`Impossible de notifier ${user.email} pour son adhésion manquante :\n${error.message}`)
        .catch(notifyError => {
          // Don't throw an error if the notification failed
          console.debug('Unable to notify about missing membership email notification', notifyError)
        })
    })
  }
}
...
async function notifyDepletedBalance(user, newHeartbeat) {
  ...
  if (isBalanceDepleted) {
    ...
    const activeSubscriptions = await Subscription.findActiveSubscriptionsByDate(newHeartbeatDate, user._id)
    const hasActiveSubscription = activeSubscriptions.length > 0

    if (!hasActiveSubscription) {
      const depletedBalanceEmail = await renderDepletedBalance(user)
      await sendMail(depletedBalanceEmail, [user.email])
    } else if (user.profile.balance < 0) { // User needs to settle its debt
      const settleBalanceDebtEmail = await renderSettleBalanceDebt(user)
      await sendMail(settleBalanceDebtEmail, [user.email])
    }
  }
}
...
export function computeMemberCapabilitiesFromUser(member, user) {
  const defaultCapabilities = ['UNLOCK_GATE', 'PARKING_ACCESS', 'WIFI_CREDENTIALS_ACCESS', 'AIR_CONDITIONING_ACCESS']
