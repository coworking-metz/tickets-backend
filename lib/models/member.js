import {differenceInMinutes, endOfDay, isBefore, isSameDay, isWithinInterval, parseISO, setYear, sub} from 'date-fns'
...
export async function recomputeBalance(memberId) {
  ...
  // Ensure activity with amount is recomputed and cached
  await computeMemberActivity(memberId)
...
export function computeMemberCapabilitiesFromUser(member, user) {
  const defaultCapabilities = ['UNLOCK_GATE', 'PARKING_ACCESS']
...
export async function computeMemberActivity(memberId) {
  console.log(`Computing activity for member ${memberId}`)
  const activity = await Activity.getMemberRawActivity(memberId)
  ...
  // Reverse() to have the older ticket first
  const singleTickets = ticketsOrders.reverse().reduce((acc, order) => [ // eslint-disable-line unicorn/no-array-reduce
    ...acc,
    ...(Array.from({length: order.count}, () => ({
      ...order,
      count: 1,
  ...
  const computedActivity = activity.map(a => {
    const duration = a.overrideValue ?? a.value
    const coveredBySubscription = subscriptions.find(s => isWithinInterval(parseISO(a.date), {
    ...
    if (coveredBySubscription) {
      const subscriptionActivity = {
        ...
        date: a.date,
        value: duration,
        amount: duration * coveredBySubscription.dailyAmount
      }
      return subscriptionActivity
    }

    const ticketActivity = consumeTicketsByActivity(singleTickets, {
      type: 'ticket',
      date: a.date,
      value: duration,
      amount: 0,
  ...
  await cache.set(getActivityCacheKey(memberId), {
    activity: computedActivity,
    expired: endOfDay(new Date()).toISOString()
  })

  return computedActivity
}

export async function getComputedMemberActivity(memberId) {
  const cacheKey = getActivityCacheKey(memberId)

  if (await cache.has(cacheKey)) {
    const cached = await cache.get(cacheKey)
    const now = new Date()
    if (isBefore(now, parseISO(cached.expired))) {
      return cached.activity
    }
  }

  return computeMemberActivity(memberId)
}

export async function getComputedMemberActivityAt(memberId, date) {
  const memberActivity = await getComputedMemberActivity(memberId)
  const activity = memberActivity.find(a => a.date === date)

  if (!activity) {
    const computedMemberActivity = await computeMemberActivity(memberId)
...
/**
 * Recursively consume tickets orders
 * with the given activity
 *
 * @param {Array} tickets Array of single tickets, each with unitAmount and count=1
 * @param {Object} activity Single activity to consume tickets for, with a debt to pay
 * @returns the activity with amount and debt updated
 */
function consumeTicketsByActivity(tickets, activity) {
  ...
  const duration = activity.debt?.value
  if (!duration) {
    return {
      ...activity,
  ...
  if (ticket.count >= duration) {
    // Ticket can cover the full activity
    if (ticket.count > duration) {
      // Put back the remaining part of the ticket
      tickets.unshift({
        ...ticket,
        count: ticket.count - duration
      })
    }

    return {
      ...activity,
      amount: duration * ticket.unitAmount,
      debt: null // Clear debt
    }
  }
  ...
  const remainingActivity = {
    ...activity,
    amount: ticket.count * ticket.unitAmount,
    // Reduce the remaining debt
    debt: {
      value: duration - ticket.count,
      amount: (duration - ticket.count) * Ticket.MISSING_TICKET_PRICE_IN_EUR
    }
  }
