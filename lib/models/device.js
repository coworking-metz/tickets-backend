...
export async function addMemberDevice(memberId, macAddress, name, type = 'UNKNOWN') {
  if (!isMacAddress(macAddress)) {
    throw createHttpError(400, `Adresse MAC invalide : ${macAddress}`)
  }
...
export async function updateMemberDevice(memberId, deviceId, macAddress, name, type) {
  if (!isMacAddress(macAddress)) {
    throw createHttpError(400, `Adresse MAC invalide : ${macAddress}`)
  }
  ...
  const updatedDeviceDocument = await mongo.db.collection('devices').findOneAndUpdate(
    {_id: new ObjectId(deviceId), member: memberId},
    {$set: {macAddress, name, type, updated: new Date()}},
...
export async function removeMemberDevice(memberId, deviceId) {
  await mongo.db.collection('devices').updateOne(
    {_id: new ObjectId(deviceId), member: memberId},
