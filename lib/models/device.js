...
export async function getAssignedDevices(date = null) {
  const devices = await mongo.db.collection('devices').find({member: {$ne: null}}).toArray()

  if (!date) {
    return devices
  }

  const activities = await Activity.getActivitiesWithDevicesForDate(date)
  ...
  for (const activity of activities) {
    ...
    for (const device of activity.devices) {
      for (const item of devices) {
        if (item.macAddress === device) {
          item.member = activity.member
        } else {
          otherDevices.push({macAddress: device, member: activity.member})
        }
      }
    }
  }

  for (const item of otherDevices) {
    devices.push(item)
  }
