...
export const MISSING_TICKET_PRICE_IN_EUR = 8
...
export function formatTicket(ticketOrderFromDatabase) {
  return {
    ...
    unitAmount: ticketOrderFromDatabase.ticketsQuantity ? ticketOrderFromDatabase.price / ticketOrderFromDatabase.ticketsQuantity : 0
  }
}
...
export async function getMemberTicketsOrders(memberId) {
  return mongo.db.collection('tickets')
    .find({memberId})
    .sort({purchased: -1})
    .toArray()
}

export async function getTicketsOrdersByDate(date) {
  const tickets = await mongo.db.collection('tickets').find({purchaseDate: date}).toArray()
  return chain(tickets)
    .map(t => formatTicket(t))
    .orderBy(['purchased'], ['desc'])
    .value()
...
export async function addTicketsToMember(memberId, ticketsQuantity, purchase, purchaseQuantity = 1) {
  ...
  return Promise.all(payloads.map(async p => {
    await mongo.db.collection('tickets').insertOne(p)
    return p
  }))
}
...
export async function removeTicket(_id) {
  const removedTicket = await mongo.db.collection('tickets').findOneAndDelete(
    {_id},
  ...
  await mongo.db.collection('users').updateOne(
    {
      _id: removedTicket.value.memberId,
      'profile.tickets.purchaseDate': removedTicket.value.purchaseDate,
      'profile.tickets.tickets': removedTicket.value.ticketsQuantity
    },
    {
      $pull: {
        'profile.tickets': {
          purchaseDate: removedTicket.value.purchaseDate,
          tickets: removedTicket.value.ticketsQuantity
        }
      }
    }
  )

  return removedTicket.value
}
