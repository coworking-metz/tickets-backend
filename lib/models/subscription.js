...
export function formatSubscription(subscriptionFromDatabase) {
  return {
    ...
    ended: computeSubcriptionEndDate(subscriptionFromDatabase.startDate),
...
export async function addSubscriptionsToMember(memberId, startDate, purchase, purchaseQuantity = 1) {
  ...
  for (let i = 0; i < purchaseQuantity; i++) {
    ...
    const {meals, meal_price} = purchase
    if (meals) {
      payload.price -= meals * meal_price // We deduce the price paid for the meals from the total, to store only the amount paid for the subscription
      promises.push(Meals.addMealVouchersToMember(memberId, payload))
    }
