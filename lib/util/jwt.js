...
export async function createAccessToken(user, wordpressUser, impersonatedBy = null) {
  const payload = {
    ...
    roles: wordpressUser.roles.filter(role => role === 'admin'
      ? Boolean(user?._id) // Forbid non-members to have admin role even if they have it in wordpress
      : true
    ),
  ...
  if (user) {
    ...
    const enabledCapabilities = Object.keys(memberCapabilities)
      .filter(capability => Boolean(memberCapabilities[capability]) && capability !== 'PARKING_ACCESS')
    payload.capabilities = enabledCapabilities
  } else if (wordpressUser.roles.includes('external') || isUserOnboardingToday) {
    const capabilitiesForExternalUser = [
      'UNLOCK_GATE'
      // 'PARKING_ACCESS'
    ]
    payload.capabilities = capabilitiesForExternalUser
  }
