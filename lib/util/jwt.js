...
export function createAccessToken(user, wordpressUser) {
  ...
  if (wordpressUser.visite?.date) {
    ...
    const wordpressOnboardingDate = new Date(wordpressUser.visite.date)
    const timezonedOnboardingDate = wordpressOnboardingDate.toLocaleString('en-US', {timeZone: 'Europe/Paris'})
    const onboardingDate = new Date(timezonedOnboardingDate)
    onboardingDate.setHours(wordpressOnboardingDate.getHours())
