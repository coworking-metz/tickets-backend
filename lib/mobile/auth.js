...
const {MOBILE_APP_BASE_URL, TICKETS_BASE_URL} = process.env
// TODO: find the host with req.get('host') instead of hardcoding it
const CALLBACK_URI = `${TICKETS_BASE_URL}/api/mobile/v1/auth/callback`

const login = (req, res, next) => {
  const callbackURL = new URL(CALLBACK_URI)

  // TODO: should only be allowed in local
  if (req.query.redirect) {
    // Encode base64 without padding to avoid passport to reject the redirect url
    const base64redirect = Buffer.from(req.query.redirect).toString('base64').replace('=', '')
