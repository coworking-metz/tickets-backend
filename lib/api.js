...
import {createFlag, getFlagResponse, getFlagUrl} from './util/flags.js'
...
export async function presenceWebhook(req, res) {
  ...
  if (action === 'mac') {
    ...
    const flagUrl = getFlagUrl(flagId, getServerBaseUrl(req))
    return res.send({flagId, flagUrl})
  }

  // We want to trigger the reupload of presences for the given date range
  if (action === 'daterange') {
    const {start, end} = req.body
    const flagId = await createFlag('presences-daterange', {start, end})
    const flagUrl = getFlagUrl(flagId, getServerBaseUrl(req))
    return res.send({flagId, flagUrl})
  }
