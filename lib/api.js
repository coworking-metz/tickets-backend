...
export async function getMemberSubscriptions(req, res) {
  ...
  const memberActivity = await Activity.getMemberRawActivity(req.rawUser._id)
  const subscriptionsWithActivity = subscriptions.map(subscription => {
    const activitiesCoveredBySubscription = memberActivity
      .filter(a => isWithinInterval(parseISO(a.date), {
        start: parseISO(subscription.started),
        end: parseISO(subscription.ended)
      }))

    const totalActivityWithinSubscription = activitiesCoveredBySubscription
    ...
    return {
      ...
      attendanceCount: activitiesCoveredBySubscription.length,
      savingsOverTickets
    }
  })
...
export async function addMemberSubscription(req, res) {
  ...
  const {started, orderReference, comment, amount, purchased, ended} = req.body
  if (!isValid(new Date(started))) {
    throw createHttpError(400, `Invalid start date ${started}`)
  }

  if (!isValid(new Date(ended))) {
    throw createHttpError(400, `Invalid end date ${ended}`)
  }

  if (isAfter(new Date(started), new Date(ended))) {
    throw createHttpError(400, `Start date ${started} should be before end date ${ended}`)
  }
  ...
  const startDate = new Date(started).toISOString().slice(0, 10)
  const endDate = new Date(ended).toISOString().slice(0, 10)

  const insertedSubscription = await Subscription.addSubscriptionToMember(
    memberId,
    startDate,
    endDate,
    {
...
export async function updateMemberSubscription(req, res) {
  ...
  const {comment, amount, orderReference, purchased, started, ended} = req.body
  if (!isValid(new Date(started))) {
    throw createHttpError(400, `Invalid start date ${started}`)
  }
...
export async function addMemberTicketsOrder(req, res) {
  const memberId = req.rawUser._id
  const {count, orderReference, comment, amount, purchased} = req.body
  ...
  if (isNil(count) || count < 0) {
    throw createHttpError(400, 'Invalid count')
  }
