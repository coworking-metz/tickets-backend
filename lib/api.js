...
import * as Purchases from './models/purchases.js'
...
export async function purchaseWebhook(req, res) {
  ...
  await Promise.all(items.map(async item => {
    let storePurchase = false
    const {quantity, product_type: productType} = item
    if (productType === 'ticket-unite') {
      await Ticket.addTicketsToMember(userId, purchaseDate, quantity)
      storePurchase = true
    }

    if (productType === 'carnet-tickets') {
      await Ticket.addTicketsToMember(userId, purchaseDate, quantity * 10)
      storePurchase = true
    }

    if (productType === 'abonnement') {
    ...
    if (storePurchase) {
      await Purchases.storeProductPurchase(userId, orderReference, item)
    }
  }))
