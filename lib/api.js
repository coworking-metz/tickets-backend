...
import {createAccessToken, createRefreshToken, verifyRefreshToken} from './util/jwt.js'
import {getUser} from './util/wordpress.js'
...
export async function getUserAccessToken(req, res) {
  const jwtRefreshToken = req.body.refreshToken
  if (!jwtRefreshToken) {
    throw createHttpError(400, 'Missing refreshToken')
  }

  const userToImpersonate = req.rawUser
  const wordpressUser = await getUser(userToImpersonate.wpUserId)

  const impersonatedUserAccessToken = await createAccessToken(userToImpersonate, wordpressUser, req.user)
  ...
  res.send({
    accessToken: impersonatedUserAccessToken,
