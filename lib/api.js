...
export async function resolveUser(req, res, next) {
  if (req.params.userId) {
    req.rawUser = await mongo.db.collection('users').findOne({wpUserId: req.params.userId})
    return next()
  }
  ...
  req.rawUser = await mongo.db.collection('users').findOne({
...
export async function getUserStats(req, res) {
  const userStats = computeUserStats(req.rawUser)
  res.send(userStats)
}

export async function getUserPresences(req, res) {
  const presences = sortBy(req.rawUser.profile.presences || [], 'date')
    .reverse()
    .map(p => ({
      ...p,
      type: isPresenceDuringAbo(p.date, req.rawUser.profile.abos) ? 'A' : 'T'
    }))
