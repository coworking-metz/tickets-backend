...
import * as Meals from './models/meals.js'
...
export async function getMemberMealVouchersStats(req, res) {
  const {month} = req.params
  const date = new Date(month)
  const stats = await Meals.getMealsStats(date)
  date.setMonth(date.getMonth() - 1)
  stats.lastMonth = await Meals.getMealsStats(date)
  res.send(stats)
}

export async function getMemberMealVouchers(req, res) {
  const meals = await Meals.getMemberMealVouchers(req.rawUser._id)

  res.send(meals)
}

export async function getMemberMealVouchersActivity(req, res) {
  const meals = await Meals.getMemberMealVouchersActivity(req.rawUser._id)

  res.send(meals)
}

export async function addMealsActivity(req, res) {
  const {mealDate} = req.body
  await Meals.addMealsActivity(req.rawUser._id, mealDate)
  const mealsActivity = await Meals.getMemberMealVouchersActivity(req.rawUser._id)
  res.send(mealsActivity)
}
...
export async function getMemberMemberships(req, res) {
  const tickets = await Membership.getMemberMemberships(req.rawUser._id)
  res.send(tickets)
}
...
export async function getAllAuditEvents(req, res) {
  const events = await Audit.getAllAuditEvents()
