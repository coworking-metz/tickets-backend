...
const {addTickets, addAbo, addMembership, getUserIdByEmail, isPresenceDuringAbo, computeBalance} = require('./models')
...
async function getUserStats(req, res) {
  ...
  balance.abos = sortBy(user.profile.abos, 'aboStart')
    .map(a => ({
      ...a,
      aboEnd: sub(add(new Date(a.aboStart), {months: 1}), {days: 1}).toISOString().slice(0, 10)
    }))
...
async function notify(req, res) {
  ...
  const endOfAboUsers = candidateEndOfAboUsers.filter(user => {
    const hasAboForTomorrow = user.profile.abos.some(abo => {
      return oneMonthBeforeTomorrow < abo.aboStart && abo.aboStart <= tomorrow
    })
  ...
  const outOfTicketsUsers = todayUsers.filter(user => {
    ...
    return !isDuringAbo && computeBalance(user) <= 0
  })
...
async function purchaseWebhook(req, res) {
  ...
  if (!userId) {
    console.error(`Utilisateur introuvable pour l’adresse email ${email}`)
    return res.sendStatus(200)
  }
