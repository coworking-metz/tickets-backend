...
import {addTickets, addAbo, addMembership, getUserIdByEmail, findOrCreateRelatedUserId, updateBalance} from './models.js'
...
export async function forceWordpressSync(req, res) {
  await Member.syncWithWordpress(req.rawUser._id, req.rawUser.wpUserId)
  const member = await Member.getMemberById(req.rawUser._id)
  res.send(member)
}
