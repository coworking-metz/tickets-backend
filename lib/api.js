...
export async function updateMemberSubscription(req, res) {
  ...
  res.send(Subscription.formatSubscription(updatedSubscription))
}
...
export async function updateMemberTicket(req, res) {
  const memberId = req.rawUser._id
  ...
  const formatedTicket = Ticket.formatTicket(updatedTicket)

  Audit.logAuditTrail(req.user, 'MEMBER_TICKET_UPDATE', {
    memberId,
    ticketId,
    count: formatedTicket.count,
...
export async function addMemberMembership(req, res) {
  const memberId = req.rawUser._id
  const {membershipStart, orderReference, comment} = req.body
  ...
  if (existingMembership) {
    throw createHttpError(400, `Member already has a membership for year ${newMembershipYear}`)
  }
  ...
  res.send(Membership.formatMembership(insertedMembership))
}
...
export async function updateMemberMembership(req, res) {
  ...
  const {membershipStart, comment} = req.body
  if (!comment) {
    throw createHttpError(400, 'Missing comment')
  }
  ...
  Audit.logAuditTrail(req.user, 'MEMBER_MEMBERSHIP_UPDATE', {
    memberId,
    membershipId,
    membershipStart: updatedMembership.membershipStart,
    previousMembershipStart: membership.membershipStart,
    comment
  })

  res.send(Membership.formatMembership(updatedMembership))
}
...
export async function getMemberMeta(req, res) {
  const {metaKey} = req.params

  if (!metaKey) {
    throw createHttpError(400, 'Missing meta key')
  }

  const fields = (String(process.env.USER_META_FIELDS)).split(',')
  console.log(fields)
  if (!fields.includes(metaKey)) {
    throw createHttpError(400, 'Meta key not allowed. Must one of `' + fields.join('`, `') + '`')
  }

  const metaValue = await Member.getMemberMeta(req.rawUser._id, metaKey)

  res.send({key: metaKey, value: metaValue === undefined ? '' : metaValue})
}

export async function updateMemberMeta(req, res) {
  const {metaKey} = req.params
  ...
  await Member.setMemberMeta(req.rawUser._id, metaKey, value === undefined ? '' : value)

  res.send({key: metaKey, value})
}
