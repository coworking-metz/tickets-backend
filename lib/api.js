...
async function getUsersStats(req, res) {
  const aggregateResult = await mongo.db.collection('users').aggregate([
    {
    ...
    },
    {
      $group: {
        ...
        presences: {$sum: '$profile.presences.amount'}
      }
    },
    {
      $sort: {presences: -1}
  ...
  res.send(aggregateResult.map(r => ({
    email: r.emails[0].address,
    ...pick(r, 'firstName', 'lastName', 'presences')
  })))
}
