...
export async function importOrder(order) {
  ...
  const purchaseDate = order.purchaseDate.split(' ')[0]
...
export function handleOrderProduct(product, purchases, options = {}) {
  ...
  handleOrderProductByType(tickets, 'tiketsBook', Ticket.computeTicketsQuantity, 10)
  handleOrderProductByType(tickets, 'uniqueTicket', Ticket.computeTicketsQuantity)
