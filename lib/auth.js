...
import {getServerBaseUrl} from './utils/express.js'
...
function buildOAuthCallbackUrl(req) {
  const callbackURL = new URL(`${getServerBaseUrl(req)}${req.baseUrl}/api/oauth/callback`)
...
export function oauth2Callback(req, res, next) {
  return passport.authenticate('oauth2', {callbackURL: buildOAuthCallbackUrl(req)}, (err, user) => {
    ...
    let redirectTo = `${getServerBaseUrl(req)}${req.baseUrl}/api/ping` // Default redirect url
