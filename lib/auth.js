...
export async function refreshTokens(jwtRefreshToken) {
  ...
  // Await revokeOAuthTokens(oauthRefreshToken)
  console.log({newOauthAccessToken, newOauthRefreshToken})
  ...
  router.post('/tokens', express.json(), w(async (req, res) => {
    ...
    const {accessToken, refreshToken, user} = await refreshTokens(jwtRefreshToken)
      .catch(error => {
        ...
        notifyOnSignal(`${randomMessage}\n${error.message}`).catch(notifyError => {
          // Don't throw an error if the notification failed
          console.error('Unable to notify about an error', notifyError)
        })
        console.error(error)
