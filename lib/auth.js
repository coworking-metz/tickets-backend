...
export const ensureAttending = location => async (req, res, next) => {
  if (req.isAdmin) {
    return next()
  }
  ...
  if (!member.attending) {
    throw createError(403, 'You have to be physically present to access this content')
  }

  if (location && member.location !== location) {
    throw createError(403, `You have to be physically present at ${location} to access this content`)
  }
...
export async function refreshTokens(jwtRefreshToken, jwtAccessToken) {
  ...
  return refreshOAuthTokens(oauthRefreshToken).then(async ({
    ...
    if (error.code === 'ETIMEDOUT' || (error.code === 'ERR_NON_2XX_3XX_RESPONSE' && error.response.statusCode === 500)) {
      console.warn('Wordpress is not responding, trying to refresh access token with existing access token')
      ...
      if (userFromAccessToken) {
        ...
        const wordpressUser = {
          id: userFromAccessToken.wpUserId,
          roles: userFromAccessToken.roles,
          user_email: userFromAccessToken.email,
          display_name: userFromAccessToken.name
        }
    ...
    const {accessToken, refreshToken, user} = await refreshTokens(jwtRefreshToken, jwtAccessToken)
      .catch(error => {
        if (error instanceof jwt.TokenExpiredError
          ...
          throw createError(401, 'Accès expiré')
        }
        ...
        if (error.code === 'ERR_NON_2XX_3XX_RESPONSE' && error.response.statusCode === 400) {
          ...
          if (parsedBody.error === 'invalid_grant') {
            throw createError(401, 'Accès invalide')
          }
        }
        ...
        if (error.message) {
          notifyOnSignal(`${sample(errorMessages)}\n${error.message}`).catch(notifyError => {
            ...
            console.debug('Unable to notify about /tokens error', notifyError)
          })
        }
