...
import {buildPictureUrl, getUserFromOAuthAccessToken, refreshOAuthTokens} from './util/wordpress.js'
  ...
  return (req, res, next) => passport.authenticate('oauth2', {
    ...
    if (followEncoded) {
      ...
      if (isFollowAllowed(follow)) {
        ...
        const jwtAccessToken = createAccessToken(user._id, wordpressUser.name, wordpressUser.email, wordpressUser.roles, wordpressUser.picture)
...
export async function retrieveUserFromAccessToken(req) {
  ...
  req.user = user
  req.isAdmin = user.roles.includes('administrator')
}
...
export async function refreshTokens(jwtRefreshToken) {
  ...
  const newAccessToken = createAccessToken(user._id, wordpressUser.name, wordpressUser.email, wordpressUser.roles, wordpressUser.picture)
